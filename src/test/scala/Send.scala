...
class SendTest extends FunSuite {
    ...
    test("simple msg") {
        checkMsg(
            ...
            Msg(NameClient("flow"), "/amp", List(ArgRef(1)))
        )
    }
    ...
    test("default msg") {
        checkMsg(
            "msg: {}",
            Msg(NameClient(Defaults.client), Defaults.path, Nil)
        )
    }
    ...
    test("msg list 1") {
        checkMsgList(
            ...
            List(
                Msg(NameClient("flow"), "/amp", List(PrimArg(PrimFloat(0.2f)))),
                Msg(NameClient("sampler"), "/start", List(PrimArg(PrimBoolean(false)))))
        )
    }
    ...
    test ("simple send 1") {
        checkSend(
            ...
            Send(List(Msg(NameClient("flow"), "/amp", List(ArgRef(1)))))
        )
    }
    ...
    test ("send with cases") {
        checkSend(
            ...
            Send(Nil, List("true" -> List(Msg(NameClient("flow"), "/start", Nil)), "false" -> List(Msg(NameClient("flow"), "/stop", Nil))).toMap)
        )
    }
    ...
    test ("send with cases and default") {
        checkSend(
            ...
            Send(List(Msg(NameClient("flow"), "/default", Nil)), List("true" -> List(Msg(NameClient("flow"), "/start", Nil)), "false" -> List(Msg(NameClient("flow"), "/stop", Nil))).toMap)
        )
    }
}
...
class UnwindSend extends FunSuite {
    ...
    def elem(n: Int, res: String) = n.toString -> List(Msg(NameClient(Defaults.client), Defaults.path, List(PrimArg(PrimString(res)))))

    test ("unwind int") {
        check("send: { int: [a, b, c] }", Send(Nil, List(elem(0, "a"), elem(1, "b"), elem(2, "c")).toMap))
    }

    def elem(x: Boolean, res: String) = x.toString -> List(Msg(NameClient(Defaults.client), Defaults.path, List(PrimArg(PrimString(res)))))

    test ("unwind boolean") {
        check("send: { bool: [a, b] }", Send(Nil, List(elem(true, "a"), elem(false, "b")).toMap))
    }

    def elem(x: String, res: String) = x -> List(Msg(NameClient(Defaults.client), Defaults.path, List(PrimArg(PrimString(res)))))
    ...
    def elem2(n: Int, res: String) = n.toString -> List(Msg(NameClient(client), path, List(PrimArg(PrimString(res)))))
    ...
    def elem2(x: Boolean, res: String) = x.toString -> List(Msg(NameClient(client), path, List(PrimArg(PrimString(res)))))
    ...
    test ("unwind simple send with client and path 1") {
        check("send: { client: flow, path: /amp, default: [msg: { args: []}] }", Send(List(Msg(NameClient(client), path, Nil)), Map()))
    }

    def msg(n: Int) = n.toString -> List(Msg(NameClient(client), path, Nil))
    test ("unwind simple send with client and path 2") {
        check("send: { client: flow, path: /amp,  case 0: [msg: { args: []}], case 1: [msg: {args: []}],  default: [msg: { args: []}] }",
            Send(List(Msg(NameClient(client), path, Nil)), List(msg(0), msg(1)).toMap))
    }

    test ("unwind through") {
        check("send: [msg: { client: flow, path: /amp, args: []}]",
            Send(List(Msg(NameClient(client),path,Nil)), Map()))
    }
